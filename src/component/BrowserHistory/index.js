...
class BrowserHistory extends Component {
  state = {inputVal: '', historyList: initialHistoryList}
  ...
  render() {
    const {inputVal, historyList} = this.state
    ...
    return (
      <div className="main-body">
        ...
        {filteredHistory.length === 0 ? (
          <h1 className="empty-text">There is no history to show</h1>
        ) : (
          <ul className="history-container">
            {filteredHistory.map(eachHistory => (
              <HistoryItem
                key={eachHistory.id}
                siteHistory={eachHistory}
                removeHistory={this.deleteHistory}
              />
            ))}
          </ul>
        )}
